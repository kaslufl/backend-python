...
class FindPetController(RouteInterface):
    ...
    def __init__(self, find_pet_use_case: Type[FindPet]):
        self.find_pet_case_use_case = find_pet_use_case
    ...
    def route(self, http_request: Type[HttpRequest]) -> HttpResponse:
        ...
        if http_request.query:
            ...
            if "user_id" in query_string_params and "pet_id" in query_string_params:
                user_id = http_request.query["user_id"]
                pet_id = http_request.query["pet_id"]
                response = self.find_pet_case_use_case.by_pet_id_and_user_id(
                ...
                user_id = http_request.query["user_id"]
                response = self.find_pet_case_use_case.by_user_id(user_id=user_id)

            elif (
                "pet_id" in query_string_params and "user_id" not in query_string_params
            ):
                pet_id = http_request.query["pet_id"]
                response = self.find_pet_case_use_case.by_pet_id(pet_id=pet_id)
